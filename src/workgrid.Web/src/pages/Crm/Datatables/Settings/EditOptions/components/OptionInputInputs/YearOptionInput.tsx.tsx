...
export const YearOptionInput = ({ resolvedValue, inputName, formik, markChanged, changedMap, commonProps }: BaseDateInputProps) => {
  const onChange: DatePickerProps["onChange"] = (_, s) => { formik.setFieldValue(inputName, s); markChanged(); };
  return (
    <>
      <Input {...commonProps} type="hidden" />
      <DatePicker picker="year" value={resolvedValue ? dayjs(resolvedValue, "YYYY") : null} onChange={onChange} className={pickerCls(changedMap)} />
    </>
  );
};